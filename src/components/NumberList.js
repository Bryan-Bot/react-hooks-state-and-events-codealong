...
function NumberList() {
  // here state numbers is equal to an array of numbers
 
  const [numbers, setNumbers] = useState([14,9,32])
  const [filterBy, setFilterBy] = useState("All");

  // we map over {numbers } array and create li tags 
  const NumberList = numbers.map((num) => <li key={num} onClick={() => handleLiClick(num)}>{num}</li>)
